    ...
    public static class PlaneWarSceneSetup
    {
        ...
        static void FixReferences()
        {
            ...
            so.FindProperty("scoreText").objectReferenceValue =
                hudGo.transform.Find("ScoreText")?.GetComponent<Text>();
            so.FindProperty("timeText").objectReferenceValue =
                hudGo.transform.Find("TimeText")?.GetComponent<Text>();
            so.FindProperty("gameOverText").objectReferenceValue =
                hudGo.transform.Find("GameOverText")?.GetComponent<Text>();
            ...
            var playerPrefab = prefabDir != null
                ? AssetDatabase.LoadAssetAtPath<GameObject>(prefabDir + "/Player.prefab")
                : null;
            var bulletPrefab = prefabDir != null
                ? AssetDatabase.LoadAssetAtPath<GameObject>(prefabDir + "/Bullet.prefab")
                : null;
            if (playerPrefab != null && bulletPrefab != null)
            {
                ...
                playerSo.FindProperty("bulletPrefab").objectReferenceValue =
                    bulletPrefab.GetComponent<SamplePlaneWarMonoBullet>();
