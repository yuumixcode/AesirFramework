namespace Runestone.AesirArchitecture.Samples
{
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>标准档读写</b>：写入直接调用 Model 写方法（不经 Command），
    /// 读取直取只读属性 <c>Count.Value</c>——Model 封装修改入口，读写路径保持简单。</para>
    /// <para>对照：快捷档（Counter-Mvp-Quick）Presenter 直改可写 ObservableValue；
    /// 严格档（Counter-Mvp-Strict）写入改走 Command、读取改走 Query——
    /// 与 MVC-2（Counter-Mvc-Standard）Controller 直调写方法分级一致，差异仅在刷新路径（Presenter 推送 vs View 订阅）。</para>
