...
namespace Runestone.AesirInspector.OdinIntegration.Editor
{
    ...
    {
        static readonly Dictionary<Type, SourceFileEntry[]> _sourceFilesCache = new();

        static readonly Regex _typeDefinitionRegex = new(
            @"\b(class|struct|enum|interface)\s+(\w+)", RegexOptions.Compiled);

        static readonly Regex _memberDeclRegex = new(
        ...
        static readonly Regex _leadingAttributesRegex = new(
            @"^(\s*\[.*?\]\s*)+", RegexOptions.Compiled);

        static readonly HashSet<string> _declarationKeywords = new(StringComparer.Ordinal)
        {
        ...
        public static SourceFileEntry[] GetSourceFiles(Type type)
        {
            ...
            if (_sourceFilesCache.TryGetValue(type, out var cached)) return cached;
        ...
        public static SourceFileEntry[] FindSourceFiles(Type type)
        {
            ...
            foreach (var guid in guids)
            {
                ...
                if (scriptClass == searchType || (scriptClass == null && monoScript.name == typeName))
                {
                    ...
                    if (!isPartial) return results.ToArray();
                }
            }
        ...
        public static string ExtractMemberName(string declarationLine)
        {
            ...
            if (genericMethodMatch.Success && !_declarationKeywords.Contains(genericMethodMatch.Groups[1].Value))
