...
namespace Runestone.AesirInspector.OdinIntegration.Editor
{
    ...
    public static class SourceSummaryInitializer
    {
        ...
        static readonly Dictionary<Type, Dictionary<string, string>> _summaryCache = new();
        ...
        static string ResolveSummary(MemberInfo member)
        {
            ...
            if (!_summaryCache.TryGetValue(declaringType, out var summaries))
            {
                ...
                summaries = SourceSummaryParser.ParseSummaries(entries, declaringType.Assembly.GetName().Name);
            ...
            if (plusIndex >= 0)
            {
                ...
                if (lastDot >= 0)
                    typeFullName = typeFullName.Substring(0, lastDot + 1) + typeFullName.Substring(plusIndex + 1);
                else
