...
namespace Runestone.AesirInspector.OdinIntegration.Editor
{
    ...
    {
        static readonly Regex _summaryContentRegex = new(
            @"<summary>\s*(.*?)\s*</summary>",
            RegexOptions.Singleline | RegexOptions.Compiled);

        static readonly Regex _xmlTagRegex = new(
            @"<see\s+cref=""([^""]*)""\s*/?>|<[^>]+>",
            RegexOptions.Compiled);

        static readonly Regex _multiSpaceRegex = new(@"  +", RegexOptions.Compiled);

        static readonly Regex _typeDeclRegex = new(
            @"\b(class|struct|enum|interface)\s+(\w+)",
            RegexOptions.Compiled);

        static readonly Regex _namespaceRegex = new(
            @"^\s*namespace\s+([\w.]+)",
            RegexOptions.Compiled);
        ...
        public static Dictionary<string, string> ParseSummaries(SourceFileEntry[] entries, string assemblyName = null)
        {
            ...
            {
                if (entry?.SourceLines == null) continue;
        ...
        static Dictionary<string, string> ParseSummariesFromLines(string[] lines, string keyPrefix = "")
        {
            ...
            while (i < lines.Length)
            {
                ...
                else
                {
                    ...
                    var paramPart = ExtractParameterTypesFromDecl(declLine);
                    if (paramPart != null)
                        key += "(" + paramPart + ")";
                }
        ...
        static string ExtractParameterTypesFromDecl(string declLine)
        {
            ...
            // 无参数方法
            if (string.IsNullOrEmpty(paramSection)) return "";
            ...
            foreach (var param in paramParts)
            {
                ...
                // 移除修饰符
                p = System.Text.RegularExpressions.Regex.Replace(p, @"^(this\s+|params\s+|ref\s+|out\s+|in\s+)+", "");
        ...
        {
            if (string.IsNullOrEmpty(line)) return inBlockComment;
