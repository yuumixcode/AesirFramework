...
{
    /// <summary>
    /// 为 <see cref="AudioModule" /> 提供 Odin Inspector 属性处理器，
    /// 动态注入 DDOL 开关的说明信息框，使 Runtime 程序集零 Odin 依赖。
    /// </summary>
    public class AudioModuleAttributeProcessor : OdinAttributeProcessor<AudioModule>
    {
        /// <summary>
        /// DDOL 开关的取值含义说明文案（字段级 Info 信息框内容）。
        /// </summary>
        /// <remarks>
        /// 与 UIModule 的开关语义一致：仅预放置为根物体时生效，
        ...
        {
            if (member.Name == AudioModule.DontDestroyOnLoadFieldName)
            {
