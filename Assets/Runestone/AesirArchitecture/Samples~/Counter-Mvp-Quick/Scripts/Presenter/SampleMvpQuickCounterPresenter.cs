namespace Runestone.AesirArchitecture.Samples
{
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>快捷档写入</b>：Presenter 直接修改 Model 的可写 ObservableValue
    /// （<c>count.Value++</c>，不建 Command、不经写方法），读取直取 <c>count.Value</c>——
    /// 与 MVC-1（Counter-Mvc-Quick）的 View 兼 Controller 直改写法一致。</para>
    /// <para><b>这是快捷写法</b>：绕过写方法直改 ObservableValue，适合原型 / 小功能；
    /// 标准档（Counter-Mvp-Standard）收窄为只读暴露 + 写方法；
    /// 严格档（Counter-Mvp-Strict）再加接口注册 + Command 写入 + Query 读取。</para>
    /// <para><b>快捷档不建 View 接口</b>：Presenter 直接持有具体面板类（与 MVC-1 无任何接口抽象一致）；
    /// 标准档起 View 契约才以接口（<c>IXxxView</c>）形式存在。</para>
