...
namespace Runestone.AesirArchitecture.Tests.Editor
{
    ...
    public class QuickCreateSOMenuItemTests
    {
        ...
        {
            Assert.IsTrue(QuickCreateSOMenuItem.IsCreatableSoClass(typeof(NormalSO)), "非抽象 SO 子类应可创建");
            Assert.IsFalse(QuickCreateSOMenuItem.IsCreatableSoClass(typeof(AbstractSO)), "抽象 SO 不可实例化，应被过滤");
            Assert.IsFalse(QuickCreateSOMenuItem.IsCreatableSoClass(typeof(NotSO)), "非 SO 类型应被过滤");
            Assert.IsFalse(QuickCreateSOMenuItem.IsCreatableSoClass(null), "null 类型应被过滤");
        }
        ...
        public void GetDefaultAssetName_TrimsSoSuffix()
        {
            ...
            Assert.AreEqual("So", QuickCreateSOMenuItem.GetDefaultAssetName("So"), "小写 so 不裁剪");
            Assert.AreEqual("SomeSOX", QuickCreateSOMenuItem.GetDefaultAssetName("SomeSOX"), "SO 不在词尾不裁剪");
        }
