namespace Runestone.AesirArchitecture.Samples.MvcStrict
{
    ...
    /// <para>
    /// 双接口设计：业务接口 <see cref="ISampleMvcStrictCounterController" /> 是 View 侧的持有类型
    /// （只暴露业务操作）；框架角色接口 <c>IController&lt;T&gt;</c> 提供 Command / Query 能力——
    /// 两者各司其职，读写分离在类型层面闭环。
    /// </para>
    /// <para>
    /// 读取口径：原始值 View 直接用只读订阅；加工值（十位近似）经 <c>ExecuteQuery</c> 查询——
    /// Query 只在需要加工修饰（四舍五入等）时使用，返回原始值则无需 Query。
    /// </para>
    /// <para>对照：标准档（Counter-Mvc-Standard）Controller 由 View 注入共享 Model、直调写方法。</para>
    /// <para>
    /// 数据流：View（用户输入）→ Controller → Command → Model 写方法 → ObservableValue 通知 → View 刷新；
    /// View 需加工值时 → Controller → Query（只读加工）→ 返回。
    /// </para>
    /// </remarks>
