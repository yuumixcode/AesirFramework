namespace Runestone.AesirArchitecture.Samples.MvcStrict
{
    ...
    /// <remarks>
    ///     <para>
    ///     <b>严格档暴露面</b>：对外只暴露只读 <see cref="IReadOnlyObservableValue{T}" />
    ///     + 显式写方法（<see cref="Increase" /> / <see cref="Decrease" /> / <see cref="Reset" />）——
    ///     外部（View / Query）只能订阅与读取，修改必经 Model 的写方法；
    ///     Command 内部也经写方法修改，不直接碰 ObservableValue 的值。
    ///     </para>
    ///     <para>
    ///     对照：快捷档（Counter-Mvc-Quick）直接暴露可写 ObservableValue；
    ///     标准档（Counter-Mvc-Standard）只读暴露 + 写方法但不做接口抽象（具体类注册）。
    ///     </para>
    /// </remarks>
