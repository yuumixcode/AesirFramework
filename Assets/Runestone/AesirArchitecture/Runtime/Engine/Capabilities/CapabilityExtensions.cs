...
namespace Runestone.AesirArchitecture
{
    ...
    public static class CapabilityExtensions
    {
        ...
        /// 以下两种情况会抛出此异常：
        /// <para>1. 目标 Model 未在 Context 中注册——需在 <c>Configure()</c> 中提前调用 <c>RegisterModel&lt;T&gt;()</c>。</para>
        /// <para>2. 目标 Model 已注册但尚未初始化——通常表示注册顺序错误或存在循环依赖，被依赖的 Model 应先注册。</para>
        /// </exception>
        ...
        /// <exception cref="InvalidOperationException">
        /// 以下两种情况会抛出此异常：
        /// <para>1. 目标 Service 未在 Context 中注册——需在 <c>Configure()</c> 中提前调用 <c>RegisterService&lt;T&gt;()</c>。</para>
        /// <para>2. 目标 Service 已注册但尚未初始化——通常表示注册顺序错误或存在循环依赖，被依赖的 Service 应先注册。</para>
        /// </exception>
