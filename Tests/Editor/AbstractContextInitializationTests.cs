...
namespace Runestone.AesirArchitecture.Tests.Editor
{
    ...
    public class AbstractContextInitializationTests
    {
        ...
        public void Interface_InitializeThrows_DoesNotCacheSingleton()
        {
            ...
            Assert.AreEqual(2, CountingModel.InitializeCount, "每次访问都重新创建上下文并初始化 CountingModel，共 2 次");
            Assert.AreEqual(0, CountingModel.DisposeCount, "失败路径不做回滚 Dispose，半成品模块随实例交由 GC 回收");
            AesirArchitectureDebug.LogTestInfo("Interface(初始化失败): 不缓存单例，根因异常每次抛出，无回滚");
        }
        ...
        class ThrowingModelContext : AbstractContext<ThrowingModelContext>
        {
            protected override void Configure()
            {
                ...
                RegisterModel(new ThrowingModel());
            }
        }
