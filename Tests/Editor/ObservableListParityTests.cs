...
namespace Runestone.AesirArchitecture.Tests.Editor
{
    ...
    public class ObservableListParityTests
    {
        ...
        public void Mutation_MatchesObservableCollection()
        {
            ...
            list.Move(3, 1); reference.Move(3, 1);
            Equal(10, 20, 50, 30, 40);

            list.Insert(2, 99); reference.Insert(2, 99);
            Equal(10, 20, 99, 50, 30, 40);

            list.RemoveAt(2); reference.RemoveAt(2);
            Equal(10, 20, 50, 30, 40);

            list[3] = 88; reference[3] = 88;
            Equal(10, 20, 50, 88, 40);

            list.Clear(); reference.Clear();
            Equal(new int[0]);

            // BCL 无 Range 重载，逐项施加同一变更后比对
            list.AddRange(new[] { 100, 200, 300 });
            reference.Add(100); reference.Add(200); reference.Add(300);
            Equal(100, 200, 300);

            list.InsertRange(1, new[] { 400, 500, 600 });
            reference.Insert(1, 400); reference.Insert(2, 500); reference.Insert(3, 600);
            Equal(100, 400, 500, 600, 200, 300);

            list.RemoveRange(2, 2);
            reference.RemoveAt(2); reference.RemoveAt(2);
