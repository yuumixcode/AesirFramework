    ...
    public static class BackgroundCommandPump
    {
        ...
        private static void WorkerLoop(Control control)
        {
            while (control.Running)
            {
                ...
                // No Running check inside: once dequeued, a command must be executed or answered
                // — dropping it would strand its native request. Stop() joins this thread, so the
                // batch finishes before the drain runs.
                while (_backgroundQueue.TryDequeue(out var cmd))
                {
