...
namespace UnityTcp.Editor.Tools
{
    ...
    ///     durationSeconds (float, 1-15) – Recording duration (default 4).
    ///     fps             (int, 5-30)   – Output MP4 frame rate (default 15).
    ...
    ///     ffmpeg is resolved only from the trusted CODELY_FFMPEG_PATH environment variable,
    ///     known install locations, or PATH. Command parameters cannot select an executable.
    ...
    public static class ManageScreenshot
    {
                ...
                switch (action)
                {
                    ...
                    case "capture":
                        captureResult = Response.Error(
                            $"'{action}' is disabled. Use record_game_view on execute_csharp_script " +
        ...
        private sealed class Mp4RecordingSession
        {
            ...
            public CancellationTokenSource EncodeCancel;
            public readonly ManualResetEventSlim EncodeCompleted = new ManualResetEventSlim(false);
        }

        internal delegate bool Mp4EncodeRunner(
            string file,
            string args,
            string workingDir,
            out string stdout,
        ...
        internal static Mp4EncodeRunner Mp4EncodeOverride;
            ...
            JObject p, Mp4RecordingOwner owner, out Mp4RecordingHandle handle, out object error)
        {
            ...
            if (p?["ffmpeg_path"] != null)
            {
                error = Response.Error(
                    "'ffmpeg_path' is not accepted from command input. Configure a trusted absolute " +
                    "path with CODELY_FFMPEG_PATH, or install ffmpeg on PATH.");
                return false;
            }
            ...
            if (!EditorApplication.isPlaying)
            {
                error = Response.Error(
                    ...
                    "Enter play first, then pass record_game_view on execute_csharp_script " +
            ...
            string ffmpegPath = ResolveFfmpegPath(out string ffmpegError);
            if (string.IsNullOrEmpty(ffmpegPath))
            {
                error = Response.Error(ffmpegError);
                return false;
            }
            ...
            try
            {
                ...
                string tempDirectory = Path.Combine(
                    Path.GetDirectoryName(Application.dataPath), "Temp", "CodelyMp4", id);
                ...
                CodelyLogger.Log(
                    ...
                    $"{duration:F2}s @ {fps}fps, scale={scale:F2}");
        ...
        internal static bool TryGetActiveGameViewMp4RecordingConflict(out object error)
        {
            ...
            string resolution = s_Mp4Recording.Owner == Mp4RecordingOwner.Split
                ...
                : "Wait for the owning execute_csharp_script command to collect it automatically.";
            error = Response.Error(
        ...
        private static void TickGameViewMp4Recording()
        {
            ...
            if (session == null || session.Finished || session.Encoding)
            {
        ...
        private static void TickGameViewMp4RecordingBody(Mp4RecordingSession session)
        {
            ...
            if (!EditorApplication.isPlaying)
            {
                BeginMp4Encoding(session, "Play Mode ended before the requested recording duration.");
                return;
            }
            ...
            try
            {
                ...
                else
                {
                    ...
                    string framePath = Path.Combine(
                        session.TempDirectory, $"frame_{session.CapturedFrames:D6}.png");
            ...
            JObject p, Action<object> setResult)
        {
            ...
            if (session.Owner != Mp4RecordingOwner.Split)
            {
                setResult(Response.Error(
                    ...
                    "execute_csharp_script; that command collects its recording automatically."));
                yield break;
            }
        ...
        internal static void BeginActiveMp4EncodingForTests(string warning = null)
        {
            BeginMp4Encoding(s_Mp4Recording, warning);
        }

        internal static bool IsActiveMp4RecordingFinishedForTests()
        {
            return s_Mp4Recording == null || s_Mp4Recording.Finished;
        }

        internal static bool WaitForActiveMp4EncodeForTests(int timeoutMs)
        {
            return s_Mp4Recording?.EncodeCompleted.Wait(timeoutMs) ?? true;
        }

        private static void BeginMp4Encoding(Mp4RecordingSession session, string warning)
        {
            if (session == null || session.Finished || session.Encoding) return;
            EditorApplication.update -= TickGameViewMp4Recording;
            ...
            string inputPattern = Path.Combine(session.TempDirectory, "frame_%06d.png");
            string args =
                $"-y -hide_banner -loglevel error -framerate {session.Fps} " +
                $"-start_number 0 -i {QuoteProcessArgument(inputPattern)} " +
                "-vf \"scale=trunc(iw/2)*2:trunc(ih/2)*2\" " +
                "-c:v libx264 -preset veryfast -crf 23 -pix_fmt yuv420p " +
                $"-movflags +faststart {QuoteProcessArgument(session.OutputPath)}";

            session.Encoding = true;
            session.EncodeCancel = new CancellationTokenSource();
            session.EncodeCompleted.Reset();
            CancellationToken cancel = session.EncodeCancel.Token;
            Mp4EncodeRunner runner = Mp4EncodeOverride ?? DefaultMp4Encode;
            ...
            ThreadPool.QueueUserWorkItem(_ =>
            {
                try
                {
                    ...
                    if (cancel.IsCancellationRequested || s_Mp4Recording != session)
                        return;

                    if (!encoded || !File.Exists(session.OutputPath))
                    {
                        string detail = string.IsNullOrWhiteSpace(stderr)
                            ? (string.IsNullOrWhiteSpace(stdout)
                                ? "ffmpeg returned no diagnostic output."
                                : stdout.Trim())
                            : stderr.Trim();
                        RememberFinishedMp4Result(session, Response.Error(
                            $"Failed to encode MP4 with ffmpeg: {detail}"));
                        return;
                    }

                    long sizeBytes = new FileInfo(session.OutputPath).Length;
                    int encodedWidth = session.Width - session.Width % 2;
                    int encodedHeight = session.Height - session.Height % 2;
                    double actualCaptureSeconds = session.ActualEndTime > session.ActualStartTime
                        ? session.ActualEndTime - session.ActualStartTime
                        : 0.0;

                    CodelyLogger.Log(
                        $"[ManageScreenshot] Saved MP4: {session.OutputPath} " +
                        $"({session.CapturedFrames} frames @ {session.Fps}fps)");
                    ...
                    RememberFinishedMp4Result(session, Response.Success("Game View MP4 recording completed.", new
                    {
                        ...
                        fps = session.Fps,
                        durationSeconds = (double)session.CapturedFrames / session.Fps,
                    ...
                    DeleteDirectoryBestEffort(session.TempDirectory);
                }
                catch (Exception e)
                {
                    if (!cancel.IsCancellationRequested && s_Mp4Recording == session)
                    {
                        RememberFinishedMp4Result(session, Response.Error(
                            $"Failed to encode MP4 with ffmpeg: {e.Message}"));
                    }
                }
                finally
                {
                    session.Encoding = false;
                    try { session.EncodeCompleted.Set(); } catch { }
                }
            });
        }

        private static bool DefaultMp4Encode(
            string file,
            string args,
            string workingDir,
            out string stdout,
            ...
            CancellationToken cancel)
        {
            return ExecPath.TryRun(
                file, args, workingDir, out stdout, out stderr, timeoutMs, extraPathPrepend: null, cancel);
        }

        private static string ResolveFfmpegPath(out string error)
        {
            error = null;
            string configured = Environment.GetEnvironmentVariable("CODELY_FFMPEG_PATH");
            if (!string.IsNullOrWhiteSpace(configured))
            {
                ...
                {
                    error =
                        "CODELY_FFMPEG_PATH must contain a trusted absolute executable path.";
                    return null;
                }
                ...
                {
                    error =
                        $"The ffmpeg executable configured by CODELY_FFMPEG_PATH does not exist: '{trimmed}'.";
                    return null;
                }
                ...
                {
                    error =
                        "CODELY_FFMPEG_PATH must point to a Windows .exe executable.";
                    return null;
                }
#endif
            ...
            foreach (string candidate in knownCandidates)
            {
            ...
            error =
                "ffmpeg was not found. Install ffmpeg on PATH, or set CODELY_FFMPEG_PATH " +
                "to a trusted absolute executable path.";
            return null;
        }
        ...
        private static void CleanupMp4Recording(bool preserveOutput)
        {
            ...
            if (session.EncodeCancel != null)
            {
                try { session.EncodeCancel.Cancel(); } catch { }
                try { session.EncodeCompleted?.Wait(1000); } catch { }
                try { session.EncodeCancel.Dispose(); } catch { }
            }
            try { session.EncodeCompleted.Dispose(); } catch { }
        ...
        private static void DeleteDirectoryBestEffort(string path)
        {
            ...
            {
                CodelyLogger.LogWarning(
                    $"[ManageScreenshot] Failed to clean MP4 temporary directory '{path}': {e.Message}");
            }
        }
            ...
            JObject p, Action<object> setResult)
        {
            ...
            setResult(Response.Error(
                ...
                "the same execute_csharp_script that triggers or restarts the effect. Only when " +
