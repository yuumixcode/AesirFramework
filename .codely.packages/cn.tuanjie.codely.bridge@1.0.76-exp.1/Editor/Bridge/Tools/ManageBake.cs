...
namespace UnityTcp.Editor.Tools
{
    ...
    public static class ManageBake
    {
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            try
            {
                // The Play Mode write guard for the write actions here — bake_navmesh,
                // bake_lighting, clear_navmesh, clear_baked_data — is applied centrally by
                // UnityTcpBridge before dispatch (see WriteGuard.WriteActions). Repeating it in
                // these handlers would be unreachable and would give the table a second, drifting
                ...
                switch (action)
                {
                    ...
                    case "bake_lighting":
                        return RunToCompletion(new BakeLightingJob(), @params, 600);
                    case "wait_for_bake":
                        return RunToCompletion(
                            new WaitForEditorSettledJob { What = "bake" },
                            @params, 600);
                    case "clear_navmesh":
                        return ClearNavMesh();
