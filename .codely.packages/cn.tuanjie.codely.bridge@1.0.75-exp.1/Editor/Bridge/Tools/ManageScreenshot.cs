...
namespace UnityTcp.Editor.Tools
{
    ...
    ///   capture_scene_view always captures the current editor view (WYSIWYG).
    ///   capture_asset view    – "cardinal" (default), "front", "back", "left", "right",
    ///                           "top", "bottom", "iso", or "all".
    ...
    public static class ManageScreenshot
    {
        ...
        private static IEnumerator CaptureSceneViewCoroutine(JObject p, Action<object> setResult)
        {
            ...
            // Scene View capture is WYSIWYG: always capture the current editor view.
            // Multi-angle expansion remains available to capture_asset only.
            List<string>    views    = new List<string> { "current" };
            List<Texture2D> captures = new List<Texture2D>();
