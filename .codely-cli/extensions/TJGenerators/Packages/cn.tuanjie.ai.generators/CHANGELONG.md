...
- Agent 的 3D 模型、图片、图片分层、图片放大、音视频、语音合成、声音克隆及天空盒生成改用 MCP；Unity 侧继续负责资产导入和后处理
- 本地任务查询与列表统一为 `query_local_task`、`list_local_tasks`，保留 `list_session_assets` 会话资产清单；旧任务仍可查询和恢复
- 扩展通过 skills 指导通用子代理执行，移除按资源类型划分的专属代理配置，并提供默认生成服务 MCP 连接配置

### Added

- 从 URL 导入模型、图片、音频和视频，支持图片多图导入、模型绑骨/动作后处理、音频组件绑定及绿幕视频材质
- 天空盒复用图片导入工具，支持 Cubemap 和天空盒材质，保留已有资产引用与材质设置
- 补齐语音合成与克隆音色的扩展使用指引

### Fixed

- 修复新增导入脚本的非标准 `.meta` GUID 导致 Unity 2019 忽略脚本、编译失败的问题
- 图片、音频和视频下载统一使用网络请求兼容层，避免 Unity 2020.1 引用尚未提供的 API
- 统一扩展与桌面端的 MCP 服务名，避免自动配置与扩展同时加载时重复注册工具
- 修复移除专属代理配置后，扩展打包仍依赖旧目录导致构建失败的问题
- 修复批量导入误处理 `.meta` 文件导致 Sprite 设置和动画引用丢失的问题
- 补全 UI Kit 美术图层转换为 Sprite 后再放置的指引
- 修复动作导入设置不循环时，控制器仍自动重复播放的问题
- 自动序列帧保留生成结果已有的透明通道，避免再次绿幕抠图误删绿色主体
- 生成图片与图层导入时保留非二次幂尺寸，避免默认缩放损失像素
- 修复音频导入完成后历史记录和会话资产清单仍引用已删除占位资产的问题
- 修复部分扩展流程引用旧工具名导致工具不可用的问题
- 修复任务提交或等待结束后过早报告完成的问题，完成前验证实际导入资产，支持中断后续接原任务
- 修复导入重试或域重载恢复时可能重复处理、丢失后处理设置的问题
- 下载失败时保留已有图片；替换天空盒时保持纹理和材质 GUID，避免破坏已有引用
- 修复中文工程名导致请求头设置失败的问题
- 修复部分 FBX 模型导入后自动缩放过小的问题

### Removed

- 已迁移至 MCP 的 Unity 本地生成入口，以及按资源类型重复暴露的查询、列表入口
