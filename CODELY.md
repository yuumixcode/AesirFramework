...
- [2026-08-22 03:16:45] [2026-08-22 03:18:00] RAA 0.11.0 已定稿并提交（commits 00c52a4 主体 + 0ba5910 版本定稿，2026-08-22）：①RAA CHANGELOG 的 [0.10.0] 节必须保持已发布状态——此前多轮会话误将未发布内容（六档示例家族等）并入 0.10.0 节，定稿时已用 git show HEAD 恢复并把全部新内容写入新 [0.11.0] 节；②版本同步惯例（aesir-version-sync skill）：RAA package.json + 双 README 徽章 + 根 CHANGELOG 聚合节与当前版本表 + 根 README/README_EN 版本表 + Assets/Samples/Aesir Architecture/0.11.0 文件夹对齐（meta 随移 GUID 不变）；③三包版本不锁步（Architecture 0.11.0 / Modules 0.9.0 / Inspector 0.9.0，Modules 依赖声明 0.9.0 未随升），单包独立升版合法；④RAM（AesirModules）的 DDOL+InfoBox 修改仍未提交，留待用户处理。**Why:** 用户要求提交 RAA、移除 QFramework 对比、CHANGELOG 新增版本号。**How to apply:** 后续发版照此清单执行；已发布 CHANGELOG 节不可改写；CODELY.md 正文中的旧版本路径（0.9.0/0.10.0）因记忆规则禁止直接写文件而未更新，需用户自行 /init 或手动刷新。
