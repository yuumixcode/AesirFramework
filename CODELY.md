...
- [2026-08-15 22:04:28] AesirArchitecture 缺陷修复已完成并提交（commit a09bdc8，版本 0.9.0，2026-08-15）。缺陷分析 20 项中已完成：#1/#2（InfoBox）、#3（Scene.handle 分桶）、#4（EnsureInjected + Register 期检测，120 帧轮询已按极简原则撤销）、#5（Interface 成功后赋值，回滚已撤销）、#7（GetModel/GetService 抛异常）、#9（package.json 文案修正）、#10（原 try-catch 已按极简原则撤销，统一原生 C# fail-fast）、#15（GetRegistry 删除）、#17（补 GenericLocatorTests/ObservableValueTests）。#6 ModelReplaced/ServiceReplaced 已实现后按用户裁决删除（测试场景自行处理）。#8/#11/#12/#13/#16/#18-20 文档化或不做。**Why:** 用户按缺陷文档逐项修复后复盘，以极简原则撤销过度防御。**How to apply:** 缺陷分析 20 项已全部处置完毕。


- [2026-08-15 14:31:23] 在 Codely 会话内实跑 EditMode 测试的方法：execute_csharp_script + TestRunnerApi（assemblyNames 过滤如 Runestone.AesirArchitecture.Tests.Editor，用 TaskCompletionSource + await 等待 RunFinished）。**Why:** 此 Unity 2022.3/团结版无 ITestRunnerListener，回调接口须实现 UnityEditor.TestTools.TestRunner.Api.ICallbacks；且 execute_csharp_script 禁止 Task.Result/.Wait（判定为阻塞调用）。**How to apply:** 需要验证测试真实通过（而非仅编译通过）时用此模式；PlayMode 测试不宜经此 API 跑（域重载风险），改用 unity_editor.play + 协程脚本验证同等逻辑。
- [2026-08-15 20:21:42] Odin 程序集已全部重命名（2026-08-15，三包统一）：Runtime → Runestone.{包名}.OdinInspector，Editor → Runestone.{包名}.Editor.OdinInspector（Inspector 的 .Editor 从末尾移到中间）；目录同步 OdinInspector/（顺带修正 OdinIntergration 拼写）。关键联动：AesirModules/Runtime/Common/AssemblyInfo.cs 的 InternalsVisibleTo 必须指向新程序集名（重命名程序集时 InternalsVisibleTo 是必查项）；已同步 7 个引用方 asmdef（Inspector Tests/Samples~×3/Assets Samples×3）与现状文档（AGENTS.md、各包 README/aesir-inspector.md/development.md）。C# 命名空间保持 ...OdinIntegration... 未动（用户仅要求程序集名）；CODELY.md 正文程序集表与 ScriptDocGenerator 生成文档仍含旧名，待 /init 重新生成。**Why:** 用户要求统一 Odin 程序集命名约定。**How to apply:** 新增 Odin 相关 asmdef 一律用 OdinInspector 命名。
- [2026-08-15 21:05:16] AesirArchitecture 极简化实施完成（2026-08-15，依据 Docs/AesirArchitecture-极简分析与改进计划.md，4 个确认点全按推荐执行 + 用户追加裁决）：①事件系统回原生 C# 语义 — MiniEvent/InvokeEvent/InvokeHooks 撤销逐回调 try-catch（InvokeHooks 保留 finally 维护延迟命令状态机），MiniEvent 恢复零分配直调；②Interface 改"Initialize 成功后才赋值"+ 撤销 Initialize 快照回滚（失败不缓存、根因每次抛出、无回滚 Dispose）；③撤 120 帧自愈轮询（保留 EnsureInjected + Register 期检测）；④删 ModelReplaced/ServiceReplaced（运行时替换属测试场景）+ ModuleReplacedEventArgs.cs + AbstractContextModuleReplacementTests；⑤CapabilityExtensions 删不可达 null 分支；⑥删 GenericLocator.GetRegistry()（接口+实现，无调用方）；⑦package.json 去事件总线文案、README/README_EN 增设计边界节、CHANGELOG 合并记录、计划文档裁决表同步。测试：新增 GenericLocatorTests(8)/ObservableValueTests(7)，MiniEventTests 重写含 fail-fast 锁定(5)，AbstractContextInitializationTests 扩充(6)，最终 34/34 两轮同域通过 + PlayMode 冒烟通过。注意：与另一 Codely 会话并行执行发生文件竞争（其 B4 工作与我的 P1 互相覆盖一次），最终以"合并态"收尾——我的 P1 编辑被其终版保留，其 ResetStaticsAssistant 收窄方案落地。**Why:** 用户裁决极简优先，防御低概率/自找问题改为约定杜绝。**How to apply:** 极简计划全部条目已实施完毕；后续并行会话操作同仓库时先 stat/read 核实文件新鲜度再编辑。

- [2026-08-15 21:02:23] [2026-08-15 21:05:00] 静态变量重置职责拆分（2026-08-15 终版，用户纠正后）：ResetStaticsAssistant 保留且收窄为仅服务泛型类（泛型类中的 RIOLM 被 Unity 静默跳过——2022.3.62 探针实测，非泛型正常触发，泛型不执行且无报错，这是助手存在的根本原因）；非泛型单例（MonoLifecycleProxy、RemoveListenerOnSceneUnloadedTrigger、AesirArchitecturePlayerLoop）类内 [RuntimeInitializeOnLoadMethod(SubsystemRegistration)] ResetStatics 自重置。AbstractContext<T> 用 static T _instance + 静态构造函数 Register(() => _instance = null)；测试入口 ResetStaticsAssistant.ResetForTests()（含 [Conditional("UNITY_INCLUDE_TESTS")]，注意 git 恢复只回到上次提交，未提交的 ResetForTests 曾差点丢失）。GenericLocator<T>.Global 已移除（全仓库无调用方）。同日另一 Codely 进程并行执行极简计划（撤回滚/撤 ModelReplaced/MiniEvent 回 fail-fast），AbstractContext.Interface 现为"初始化成功后才发布"语义。EditMode 34/34 两轮同域通过。**Why:** 用户最初要求移除助手，随后想起泛型 RIOLM 静默失效正是助手的存在理由，纠正为"助手只管泛型类"。**How to apply:** 泛型类静态状态 → 助手注册；非泛型 → 类内 RIOLM；勿再引入 ContextSingletonStore 类方案（已否决）。
- [2026-08-15 21:05:27] [2026-08-15 20:41:54] AesirModules UI 模块缺陷修复进度（2026-08-15，基于 Docs/AesirModules-UI模块-优化分析.md）：已完成 #1（Binder 全家桶 6 文件迁至新程序集 Runestone.AesirModules.OdinInspector，Runtime/OdinInspector/UI/Binder/，defineConstraints=ODIN_INSPECTOR，命名空间保持 Runestone.AesirModules；BinderEditorSettings 移至 Editor/OdinInspector/UI/Binder/ 命名空间改为 ...Editor.OdinIntegration）、#5（InstantiateInactive 停用态实例化，时序 挂层→Initialize→Show 内激活，Awake/OnEnable 晚于 OnInit）、#6（字典键归一化为 uiPanel.GetType()）、#7（AesirBasePanel.OnDestroy→UIModule.RemovePanelRecord 静态反清理）、#8（EventSystem 全场景 FindAnyObjectByType 检查）、#9（Build 统一走 EnsureCanvasConfig+ApplyCanvasConfig，默认 SO 静态缓存）、#10（GetLayerRoot 缺层 LogError+null）、#11（内部状态异常补 Error 日志）、#17（ShowPanel&lt;TPanel,TPayload&gt;/Show&lt;TPanel,TPayload&gt; 泛型重载）。PlayMode 实测 7 项全过（OnInit→Awake→OnEnable→OnShow 顺序、基类 Show+HideSelf 关闭复用、外部销毁重建、EventSystem 不重复）。**Why:** 用户按缺陷文档指定修复范围（只改 UI 模块、未提及点不动）。**How to apply:** 剩余未修：P0 #2/#3/#4（Binder 代码生成器覆盖写/TrimEnd/Assembly-CSharp 硬编码——迁移时保持原样未改）、P2 #12-16/#18、P3 #19-25、P4 #26-29；#6 归一化后以基类类型查询注册面板属文档化约定（XML 注释已说明），未加防御回退。
- [2026-08-15 22:04:28] AesirArchitecture 0.9.0 已提交（2026-08-15，commit a09bdc8，159 文件 +2005 −475）。核心变更：①MVC 优先定位（package.json/README 从「MVP/MVC」改为「MVC」，IController 为推荐入口）；②目录重构为三层 — Runtime/Core/（Context+MVC/MVP 核心，含 Component/ 与 Engine/）、Runtime/Modules/（Event/CustomLifecycle/Locator/Observable/Utilities 辅助模块）、Runtime/Common/（AesirArchitecture/Debug/AssemblyInfo/ResetStaticsAssistant 基础设施）；③极简化（事件 fail-fast、初始化不回滚、删 ModelReplaced/GetRegistry）；④Odin 程序集重命名 OdinIntegration→OdinInspector（三包统一）；⑤场景分桶改 Scene.handle；⑥静态重置职责拆分（非泛型类内 RIOLM，泛型经 ResetStaticsAssistant）。测试 34/34 两轮同域通过。文档：Docs/AesirArchitecture-极简分析与改进计划.md、Docs/Unity-RuntimeInitializeOnLoadMethod-指南.md。**Why:** 用户要求 MVC 优先 + 极简 + 目录重构。**How to apply:** 版本已升至 0.9.0；CODELY.md 正文版本号/目录结构/程序集表仍为 0.8.0 旧值，待 /init 重新生成。
