...
- [2026-08-05 08:21:02] ScriptDocGenerator 模块重构完成（2026-08-04 至 2026-08-05，多轮迭代）：①移除 OdinBridge 桥接层，类型名称格式化改用 `#if ODIN_INSPECTOR` + `Sirenix.Utilities` 直接调用。②ReflectionAnalyzer 全部文件迁移到 Runtime/OdinIntegration/ScriptDocGenerator/（Attributes/AnalysisData/Core），SummaryTool 迁移到 Editor/OdinIntegration/ScriptDocGenerator/Core/。③移除 OdinAutoTooltipAttributeProcessor。④重写 Summary 解析：优先 [Summary] 特性 → 源代码 XML 解析（块注释状态跟踪 + 全限定键）。⑤OdinSourceFileHelper 精简为 FindSourceFiles + ExtractMemberName + SourceFileEntry 缓存。⑥移除多 Panel 设计，回归单 ScriptDocGeneratorSO + TypeSource 枚举（新增 MultipleAssemblies 模式）+ OdinEditorWindow。⑦修复多个 bug：/// 前缀未移除、类型检测过滤过严、null 关键字缺失、ReferenceLinkURLAttribute 未格式化、泛型类型 FullName 反引号未去掉、Type 自身 summary 键重复追加成员名、表达式体泛型方法误匹配约束类型名。⑧键格式：AssemblyName.Namespace.TypeName[.MemberName(ParamTypes)]——程序集名前缀避免跨程序集冲突，方法参数类型列表区分重载（解析器从源码声明行提取参数类型，查询端从反射 ParameterInfo.GetReadableTypeName 构造匹配键）。⑨新增 SourceParsingTests.cs（31 个测试覆盖块注释假 ///、全限定键、命名空间跟踪、单行 summary、多文件合并、多行属性声明、泛型方法、表达式体泛型方法、重载方法、ExtractMemberName 边缘场景等）。总计 100 个测试全部通过。**Why:** Odin 已是强依赖，OdinBridge 降级模式是伪需求；整合模块减少跨层碎片化；多 Panel 设计过度碎片化。**How to apply:** 类型名称格式化直接用 Sirenix API + #if；ScriptDocGenerator 模块在 Runtime/OdinIntegration + Editor/OdinIntegration 两层；单 SO + TypeSource 枚举切换模式；方法 summary 键带参数类型列表。
