...
namespace Runestone.AesirArchitecture
{
    ...
    /// <list type="bullet">
    ///     <item>
    ///     <b>推荐：编辑阶段提前挂载</b>——订阅时框架经 <c>GetComponent</c> 直接复用，
    ///     避免运行时 <c>AddComponent</c> 的开销。在 Inspector 中手动添加本组件属正常且推荐的做法。
    ///     </item>
    ///     <item>
    ///     <b>兜底：运行时自动挂载</b>——未预挂载时，
    ///     <c>RemoveListenerExtensions.RemoveListenerWhenGameObjectOnDestroyed</c> 扩展方法
    ///     会在订阅时自动为本 GameObject 添加本组件（方便性兜底，付一次 <c>AddComponent</c> 开销）。
    ///     </item>
    /// </list>
