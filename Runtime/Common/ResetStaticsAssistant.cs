...
namespace Runestone.AesirArchitecture
{
    ...
    /// <remarks>
    ///     <para>
    ///     <b>适用范围</b>：仅泛型类（如 <c>AbstractContext&lt;T&gt;</c>）需要本助手——泛型类中的
    ///     <c>[RuntimeInitializeOnLoadMethod]</c> 会被 Unity 静默跳过（不执行也不报错，Unity 2022.3 实测），
    ///     无法在自身内部声明域重载重置入口，只能通过本助手在非泛型的中心位置注册重置回调。
    ///     非泛型类不要使用本助手，直接在类内声明
    ///     <c>[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)]</c> 重置方法即可。
    ///     </para>
    ///     <para>
    ///     <b>背景</b>：Unity 默认在进入 Play Mode 时进行域重置（Domain Reload），
    ///     会清空所有静态字段的状态。但关闭 Domain Reload（即 "Enter Play Mode Options" 中的 "Reload Domain" 未勾选）后，
    ///     静态字段不会自动重置，上一次 Play Mode 的残留数据可能导致意外的行为或错误。
    ///     </para>
    ///     <para>
    ///     <b>解决方案</b>：此助手通过 <c>[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)]</c>
    ///     在域加载的子系统注册阶段自动触发 <see cref="ResetStaticsAll" />，
    ///     遍历并执行所有通过 <see cref="Register" /> 注册的重置回调，手动将静态字段恢复到初始状态。
    ///     </para>
    /// </remarks>
