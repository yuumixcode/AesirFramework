...
|---|---|---|---|
| **Aesir Architecture** | Progressive MVC architecture (capability composition, Command/Query, PlayerLoop lifecycle, reactive properties) | `cn.runestone.aesir.architecture` | `0.21.0` |
| **Aesir Modules** | UI framework (Manager of Managers, 4-layer Canvas, panel lifecycle) + event module + audio management + scene management tools + script documentation generator (requires Odin) | `cn.runestone.aesir.modules` | `0.21.0` |
...
| Sub-Package | Git URL (pinned to 0.21.0) |
|---|---|
| Aesir Architecture | `https://github.com/yuumixcode/AesirFramework.git#AesirArchitecture-v0.21.0` |
| Aesir Modules | `https://github.com/yuumixcode/AesirFramework.git#AesirModules-v0.21.0` |
...
- **ObservableCollections** — high-performance observable collections and synchronized views by Cysharp (MIT). Aesir Architecture now ships a **full port** of it (nine collections / `CollectionChanged` / synchronized views / R3 extensions / Odin debug panel — see `Documentation/observable-collections.md`), so you no longer need to pull it in separately; the upstream project remains useful for line-by-line comparison or NuGet-ecosystem usage: [Cysharp/ObservableCollections](https://github.com/Cysharp/ObservableCollections)
