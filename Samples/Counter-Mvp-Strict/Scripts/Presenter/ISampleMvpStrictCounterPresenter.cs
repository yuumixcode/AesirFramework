...
namespace Runestone.AesirArchitecture.Samples.MvpStrict
{
    ...
    /// <remarks>
    ///     <para>
    ///     <b>严格档暴露面</b>：View 按此接口存储 Presenter 实例，
    ///     经接口只可获得生命周期入口（同步初始值 / 释放订阅），不感知具体实现。
    ///     </para>
    ///     <para>
    ///     <b>
    ///     不继承 <see cref="Runestone.AesirArchitecture.IPresenter" /> /
    ///     <see cref="Runestone.AesirArchitecture.IPresenter{T}" />
    ///     </b>
    ///     ：
    ...
    ///     View 无需主动调用业务方法，故接口只含生命周期方法。
    ///     </para>
    ///     <para>
    ///     <b>继承 <see cref="IDisposable" /></b>：View 在 OnDestroy 时释放 Presenter 的事件订阅
    ///     （MVP 特有：Presenter 订阅了 View 事件，需显式注销）。
    ///     </para>
    /// </remarks>
